...
//extract the word doc to a text file using 'extract_report.js'

//let filename = prompt("Enter filename for report (Must be .DOCX): ")
var dir = './reports/uncg-fall_kick_off-chapter_report.docx'

//let imgFilename = prompt("Enter filename for image(s) (Must be .JPEG, .PNG or .PDF): ")
var img = './pics/uncg-fall_kick_off-pic.jpg'
...
(async () => {
    const browser = await puppeteer.launch({
        headless: true  // set to "false" if you want to see in action
    })
    ...
    await functions.selectProgramType(page, textLines[0].slice(textLines[0].indexOf(':') + 1, textLines[0].length).trim())
